...
=== Manual or offline plugins installation

Once you have downloaded the `.rpkg` package files of the plugins on your Rudder server, you can install it offline with:

----
rudder package install <package.rpkg> <package2.rpkg> ...
----

To update the plugin, you need to reinstall it with the same command, but in the newer version.

To install the licenses, download the archive from the server (https://download.rudder.io/plugins/licenses/[https://download.rudder.io/plugins/licenses/NAME/NAME-license.tar.gz] where `NAME` is the technical account name) and install them with:

----
rudder package install <NAME-license.tar.gz>
----

You can alternatively download individual licenses with the key file, and place them under:
