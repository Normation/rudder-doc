...
* Rudder relies on an agent that needs to be installed on all hosts to audit.
  The *agent is very lightweight* (about 90 Mb of RAM at peak) and *blazingly fast*
...
Rudder is an established project project, used in companies from small to biggest-in-their-field, as well as by hobbyists or open-source users. Typical deployments manage from 10 to 15000 nodes.
