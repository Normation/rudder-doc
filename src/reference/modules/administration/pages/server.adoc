...
Error when trying to open template 'TechniqueResourceIdByPath(List(dsc, ncf, 50_techniques, TechniqueName, 1.0),TechniqueFile.ps1)'. Check that the file exists with a .st extension and is correctly committed in Git, or that the metadata for the technique are corrects.
...
`VERBOSITY_OPTION` possible values: 'error', 'warning', 'info', 'verbose', 'debug'.
