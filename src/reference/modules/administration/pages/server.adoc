...
=== Policy backup use cases

The Policy backup feature of Rudder allows to:
...
you have to test: if anything goes wrong, you can restore the configuration from that policy backup.
...
changes on the development instance, export an backup, deploy this backup on
...
In the *Administration → Maintenance -> Policy Backup* page of the Rudder Server web interface, you can export and import the configuration of Rudder Groups, Directives and Rules.
You can create either a global policy backup or a backup containing only a specific subset of the configuration, such as Groups, Rules, Directives and active techniques, or Parameters.

When a Policy Backup is created, a 'git tag' is created into `/var/rudder/configuration-repository`.
...
Currently, creating a Policy Backup also add all unstaged files in the commit in +/var/rudder/configuration-repository+.
So it can change the Techniques used by Rudder (Rudder uses the latest commit in git as a reference). You
should away ensure that there are no unstaged files before creating a policy backup to avoid any side effect.
In the future, Rudder will prevent creating a policy backup when Techniques or Generic Methods are not committed.
...
=== Policy Backup procedure

To create a Policy Backup of Rudder Rules, Groups, Directives, or make a global backup, you need to go to
the *Administration -> Maintenance -> Policy Backup* page of the Rudder Server web interface.

To perform a global backup, the steps are:

. Click on *Backup everything* - it will update the drop-down list *Choose a backup to restore* with
the latest data
. In the drop down list *Choose a backup to restore*, select the newly created backup (backups are sorted
by date), for example 2015-01-08 16:39
. Click on *Download as zip* to download an backup that will include all elements.
...
To import the backup on the target Rudder server, you can follow the following steps:
...
. Finally, in the Web interface, go to the *Administration > Maintenance > Policy Backup* page, in the *Global Backup* section (first section), select *Latest Git commit* in the drop down list, and click on *Restore everything* to restore the configuration.
...
.. Use  Rudder web interface to import the qualified policy backup.
...
You can use the Policy backup feature configuration to deploy
preconfigured instance. You would prepare first in your labs the configuration for
Groups, Directives and Rules, create an Backup, and import the Backup on the
