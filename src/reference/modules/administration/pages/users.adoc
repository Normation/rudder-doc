...
<authentication unsafe-hashes="false" hash="bcrypt" case-sensitivity="true">
  ...
  <user name="bob"    password="xxxxxxx" permissions="administration_only, node_read" tenants="zoneA"/>
  <user name="custom1" password="xxxxxxx" permissions="node_read,node_write,rule_read,rule_edit,directive_read,technique_read"/>
  <user name="custom2" password="xxxxxxx" permissions="NodeAccess,RuleAccess,directive_read,technique_read" tenants="-"/>
  <user name="audit"  password="xxxxxxx" permissions="SecurityDashboards" tenants="*"/>
</authentication>
...
The `tenants` attribute is optional. It lists tenants which the user has access to. By default, there would be full access (equivalent to `tenants="*"`, see xref:plugins:multi-tenants.adoc[the multi-tenants plugin reference documentation]).
...
In Rudder version 9.1, support for legacy hashes is dropped definitively, and the `unsafe-hashes` attribute will be removed during startup.
