...
* When a user have not logged in, by default for 90 days, the account is marked as *disabled*, you can change this value by modifying or adding the parameter `rudder.users.cleanup.account.disableAfterLastLogin` in `/opt/rudder/etc/rudder-web.properties` by following this format : https://scala-lang.org/api/3.x/scala/concurrent/duration/Duration$.html#apply-537

* A user can be *deleted* after a certain period of time, by default the parameter `rudder.users.cleanup.account.deletedAfterLastLogin` is set to 120 days. You can change this value by modifying or adding this parameter in `/opt/rudder/etc/rudder-web.properties` by following this format : https://scala-lang.org/api/3.x/scala/concurrent/duration/Duration$.html#apply-537
...
You can modify the period of time (by default 30 days) after which the purge is performed by editing or adding the parameter `rudder.users.cleanup.purgeDeletedAfter` and by following this format : https://scala-lang.org/api/3.x/scala/concurrent/duration/Duration$.html#apply-537
