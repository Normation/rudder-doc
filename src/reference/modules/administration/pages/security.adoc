...
(e.g., password reset if it may have leaked or permissions restriction), a way to immediately stop the user sessions is to disable the user (it is also enforced when deleting the user). Then, reloading the users will take those configuration changes into account.
Restarting the web application is also a safe alternative (`systemctl restart rudder-jetty`).
