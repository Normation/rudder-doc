...
* *HTTPS with syslog compatibility*: default for new installs, keeps compatibility with pre-6.0 agents by using HTTPS reporting for 6.0 agents but still allowing syslog reporting
* *HTTPS*: switches to HTTPS only, and disables syslog report forwarding, removing all Rudder-specific
