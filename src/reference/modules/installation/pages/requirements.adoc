...
* about 90 MB of RAM during an agent run
...
You can switch to xref:configuration_management.adoc#_reporting_mode_non_compliant_only[non-compliant only] reporting mode to log only non-compliance, and divide by 3 to 15 the disk space requirement.
...
compliance from node, a safe estimation of disk usage is 200MB as a baseline, plus 11MB per managed nodes. If you use
the Policy Benchmark module, the amount can climb to 150MB per managed nodes, and we recommend to lower the data retention for non-compliant reports.
