...
=== Empty lines MiniJinja templating

The new MiniJinja template engine shipped with the default Jinja2 behavior regarding newlines.
This produces empty lines for each control block, which was inconvenient and
inconsistent with other tools.

We enabled the `trim_blocks` option in 9.0.2 to get a more familiar behavior. This changes the output
of the templating, removing empty lines on the control blocks.
...
== Upgrade from Rudder 8.3 to 9.1

Upgrade from Rudder 8.3 is supported.

=== End of support for old operating systems

Rudder 9.1 no longer supports installation on the following operating systems, for servers and relayd:
...
It was not actively maintained and was removed from 9.1.
...
Users created on an older Rudder server (without changing their passwords since), or manually created could still be using unsafe hashes. They must not be used, and their support is dropped in Rudder 9.1.
...
== Upgrade from Rudder 8.2 or older to 9.1

Direct upgrades from 8.2 versions and older are no longer supported on 9.1.
If you are still running one of those, either on servers or nodes,
please first upgrade to one of the supported versions and then upgrade to 9.1.
