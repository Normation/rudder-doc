...
Usage rudder-setup (add-repository|setup-agent|setup-server|upgrade-agent|upgrade-server) <rudder_version> [<policy_server>]
  ...
  rudder_version : x.y or x.y.z or x.y-nightly or ci/x.y or lts or latest
       x.y:            the latest x.y release (ex: 7.1)
       x.y.z:          the exact x.y.z release (ex: 7.1.6)
       x.y.z~a:        the latest x.y.z pre-release where a can be alpha1, beta1, rc1... (ex: 7.2.0~rc1) 
       x.y-nightly:    the latest public x.y nightly build (ex: 7.1-nightly)
       ci/x.y.z:       the latest development x.y.z release build (ex: ci/7.1.7)
       ci/x.y.z~a:     the latest development x.y.z pre-release build (ex: ci/7.2.0~rc1)
       ci/x.y-nightly: the latest development x.y nightly build (ex: ci/7.2-nightly)
