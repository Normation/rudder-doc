...
----
msiexec /i rudder-agent-<Rudder version>.msi
----
...
----
msiexec /i rudder-agent-<Rudder version>.msi /quiet /POLICYSERVER=my.rudder.server
----
...
In case of error, logs are available in `C:\Users\vagrant\AppData\Local\Temp`. The file will be named MSIxxx.log, sort them by date to find the last one.

To get verbose logs, use msiexec's option `/l*v MyLogFile.txt`. You can then find errors by looking for `value 3`.
