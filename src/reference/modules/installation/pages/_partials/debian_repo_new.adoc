...
echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/rudder_apt_key.gpg] http://repository.rudder.io/apt/9.2/ $(lsb_release -cs) main" > /etc/apt/sources.list.d/rudder.list
...
echo "deb [arch=$(dpkg --print-architecture)] https://download.rudder.io/apt/9.2/ $(lsb_release -cs) main" > /etc/apt/sources.list.d/rudder.list
