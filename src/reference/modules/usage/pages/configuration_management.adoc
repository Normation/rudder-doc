...
see node-level properties below if needed.
...
* `${rudder.node.state}` returns the xref:usage:advanced_node_management.adoc#node-lifecycle[node lifecycle] of the node
* `${rudder.node.policyMode}` returns the xref:usage:configuration_management.adoc#_policy_mode_audit_enforce[effective policy mode] of the node
...
NOTE: These variables are not available on Windows nodes, but only on with the classic Linux/AIX agent.
