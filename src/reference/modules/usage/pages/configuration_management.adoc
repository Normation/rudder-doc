...
server goes outline. And even if Rudder is not a monitoring tool, in that case, you want to make very
clear the fact that server didn't call home recently.
...
Sometime, it's totally expected that nodes often go outline, like in the case of managing user laptops.
...
image:assets/images/edit_rudder_system_node_property.png[Edit rudder node property]
...
image:assets/images/set_compliance_expiration_policy.png[Change node compliance expiration policy]
...
image:assets/node_keep_last_compliance_view.png[Compliance kept for a non-answering node]
...
* `duration` (optional) is the time limit during which the last compliance is kept. Its value must be composed of
   a positive integer and a unit among `day`, `hour`, `minute`, `second`, `millisecond`, `nanosecond` with an
   optional ending `s`.
   You can also use the unit abbreviated form: `d`, `h`, `m`,`s`,`ms`,`µs`,`ns`.
   Unit below `minutes` should not be useful for that use case but they are given for completeness.
