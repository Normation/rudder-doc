...
An overview of `variables` is available in https://docs.rudder.io/files/rudder-cheatsheet.pdf[Rudder cheatsheet].
...
Node properties usage inside techniques should be avoided in techniques as they are only linted when used in
a directive parameters.
If needed, use parameters in your technique and fill its value with a node property to use its value inside a technique.
====
...
|`os.fullName`
|The operating system full name (like "Debian GNU/Linux 9.2 (stretch)")

|`os.version`
|The operating system version (like "9.2")
