...
As arrays are replaced, if you want to accumulate attributes by inheritance, you should 
always use JSON objects with an unique key for each element (to avoid overriding).
You can always come back to an array by using embedded `jq` with for example a command like: 
