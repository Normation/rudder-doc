...
There is two exception to that rule:

- for unique (non multi-instance) technique, the directive with the highest priority is selected.
- for multi-instance technique, if the technique metadata `POLICYGENERATION` tag has value `merged`, then all directives will be merged into *one* policy on the agent. That behavior used to be the default one until Rudder 4.3. See https://issues.rudder.io/issues/10625[Don't merge directive from same technique on generation] for more information.
