ALL_VERSIONS = 2.3 2.4 2.5 2.6 2.7 2.8 2.9 2.10 2.11 3.0 3.1 3.2 4.0 4.1 4.2 4.3 5.0 6.0 6.1 6.2 7.0 7.1 7.2 7.3 8.0 8.1 8.2 8.3 9.0 9.1
...
all: $(GENERIC_DOCS) build/history/9.2/.htaccess build/history/9.1/.htaccess build/history/9.0/.htaccess build/sites/site/.htaccess $(VERSION_ARCHIVES) test
online: site site-dev build/sites/site/.htaccess build/history/9.2/.htaccess build/history/9.1/.htaccess build/history/9.0/.htaccess $(VERSION_ARCHIVES)
...
doc-build:
	...
	cd $@ && git checkout branches/rudder/9.2 && git pull
	cd $@/src/reference && make
	cd $@ && git add -f src/reference && git commit --allow-empty -m "Build 9.2"
	cd $@ && git clean -fd
	cd $@ && git checkout branches/rudder/9.1 && git pull
	cd $@/src/reference && make
	cd $@ && git add -f src/reference && git commit --allow-empty -m "Build 9.1"
	cd $@ && git clean -fd
	cd $@ && git checkout branches/rudder/9.0 && git pull
	cd $@/src/reference && make
	cd $@ && git add -f src/reference && git commit --allow-empty -m "Build 9.0"
	cd $@ && git clean -fd
...
changelogs-build:
	...
	for version in $(ALL_VERSIONS); do \
	  ...
	  sed -i "s@version: \"9.2\"@version: \"$$version\"@" src/changelogs/antora.yml ; \
	  sed -i "s@RUDDER_VERSION = 9.2@RUDDER_VERSION = $$version@" src/changelogs/dependencies/Makefile ; \
...
build/history/9.2/.htaccess:
	mkdir -p build/history/9.2/
	echo 'Redirect /rudder-doc/reference/current/ /rudder-doc/reference/9.2/' > $@

build/history/9.1/.htaccess:
	mkdir -p build/history/9.1/
	echo 'Redirect /rudder-doc/reference/current/ /rudder-doc/reference/9.1/' > $@

build/history/9.0/.htaccess:
	mkdir -p build/history/9.0/
	echo 'Redirect /rudder-doc/reference/current/ /rudder-doc/reference/9.0/' > $@
